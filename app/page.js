...
const tabs = ['Dashboard','Members','Vows','Contributions','Expenses','AI Reminders','Stakeholder Letters']
...
export default function Home() {
  ...
  async function refresh(){
    setLoading(true)
    const [g,m,v,c,e]=await Promise.all([
    ...
    const err=g.error||m.error||v.error||c.error||e.error
    if(err) setNotice(err.message)
    setGroups(g.data||[]);setMembers(m.data||[]);setVows(v.data||[]);setContributions(c.data||[]);setExpenses(e.data||[]);setLoading(false)
  }
  ...
  return <div className="shell">
    ...
    <div className="layout"><aside className="sidebar">{tabs.map(t=><button key={t} className={`nav-btn ${tab===t?'active':''}`} onClick={()=>setTab(t)}>{t}</button>)}<div className="footer-brand">Groups: Molyko • Buea Town • Bolifamba • Muea<br/><br/>Designed by <strong>JODEL TECHNOLOGIES</strong></div></aside>
      <main className="main"><div className="hero"><div><h2>{tab}</h2><p>Transparent stewardship for the Buea regional vehicle project.</p></div><button className="btn btn-soft" onClick={refresh}>Refresh data</button></div>{notice&&<div className={`notice ${notice.includes('success')||notice.includes('recorded')||notice.includes('added')?'success':''}`}>{notice}</div>}
        {tab==='Dashboard'&&<Dashboard stats={stats} groupStats={groupStats} vows={vows}/>} 
